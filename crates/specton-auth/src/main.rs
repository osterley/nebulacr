...
use specton_common::config::{BootstrapAdmin, GitHubOidcConfig, RegistryConfig, VaultConfig};
use specton_common::errors::RegistryError;
use specton_common::models::{AccessPolicy, Action, Project, Role, Tenant, Visibility};
...
fn parse_docker_scope(scope: &str) -> Option<RequestedScope> {
    ...
    // 3 segments: tenant/project/repo (SpectonCR multi-tenant)
...
/// POST /auth/github-actions/token — Exchange a GitHub Actions OIDC token for a SpectonCR token.
async fn github_actions_token(
    ...
    // Map GitHub claims to SpectonCR subject
...
/// GET /auth/.well-known/jwks.json — Publish SpectonCR's own public key as JWKS.
...
async fn credential_exchange(
    ...
    // Validate the session token (treat as a SpectonCR JWT)
    let mut validation = Validation::new(Algorithm::RS256);
    validation.set_audience(&[&state.config.auth.audience]);
    validation.set_issuer(&[&state.config.auth.issuer]);
...
async fn auth_http_metrics_middleware(request: Request, next: Next) -> Response {
    ...
    gauge!("spectoncr_auth_http_requests_in_flight", "route" => route).increment(1.0);
    let response = next.run(request).await;
    let elapsed = started.elapsed().as_secs_f64();
    let status = response.status().as_u16();
    let class = auth_status_class(status);

    gauge!("spectoncr_auth_http_requests_in_flight", "route" => route).decrement(1.0);
    counter!("spectoncr_auth_http_requests_total",
        "route" => route, "method" => method.clone(), "status_class" => class)
    .increment(1);
    histogram!("spectoncr_auth_http_request_duration_seconds",
...
fn user_to_scim(user: &UserRecord, base_url: &str) -> ScimUser {
    ...
    let emails = if primary_email.is_empty() {
        ...
        vec![specton_common::auth::ScimMultiValue {
            value: primary_email,
    ...
    ScimUser {
        ...
        name: user
            .display_name
            .as_ref()
            .map(|n| specton_common::auth::ScimName {
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("SpectonCR Auth Service starting");
    ...
    .record(0.0);
    gauge!("spectoncr_auth_http_requests_in_flight", "route" => "token").set(0.0);
    counter!("spectoncr_auth_circuit_breaker_rejections_total", "breaker" => "noop").increment(0);
    ...
    info!(listen_addr = %addr, "SpectonCR Auth Service listening");
