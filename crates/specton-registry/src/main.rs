//! SpectonCR OCI Registry Service
//!
...
use specton_common::auth::TokenClaims;
use specton_common::config::RegistryConfig;
use specton_common::errors::RegistryError;
use specton_common::models::{Action, Role};
use specton_common::storage::{
    blob_path, manifest_path, sha256_digest, tag_link_path, tags_prefix, upload_path,
};

use specton_mirror::service::MirrorConfig as MirrorServiceConfig;
use specton_mirror::upstream::UpstreamConfig;
use specton_mirror::{MirrorScope, MirrorService};
use specton_replication::event::ReplicationEvent;
use specton_replication::failover::FailoverManager;
use specton_replication::region::{
...
struct AppState {
    ...
    ttl_reaper_control: Option<Arc<specton_ephemeral::TtlReaperControl>>,
    /// Usage / cost telemetry recorder. Always present; defaults to a
    /// no-op when `[usage]` / `SPECTONCR_USAGE__ENABLED` is disabled.
    /// Called from blob/manifest hot paths to seed the rollup pipeline.
    usage_recorder: Arc<dyn specton_cost::UsageRecorder>,
    ...
    artifact_registry: Option<Arc<specton_artifact_types::ArtifactRegistry>>,
...
async fn dashboard_auth_middleware(
    State(config): State<Arc<specton_common::config::DashboardAuthConfig>>,
    request: Request,
    next: Next,
) -> Response {
    if !config.enabled {
...
async fn request_id_middleware(mut request: Request, next: Next) -> Response {
    ...
    if response.status() == StatusCode::UNAUTHORIZED {
        ...
        if !existing_auth.starts_with("Basic ") {
            let service_name = std::env::var("SPECTONCR_AUTH_SERVICE")
                .unwrap_or_else(|_| "spectoncr-registry".to_string());
            let realm = if let Ok(ext_url) = std::env::var("SPECTONCR_EXTERNAL_URL") {
...
async fn rate_limit_middleware(
    ...
    if state.default_rate_limiter.check_key(&key).is_err() {
        counter!("spectoncr_rate_limit_rejected_total", "tenant" => key.clone()).increment(1);
...
async fn http_metrics_middleware(request: Request, next: Next) -> Response {
    ...
    gauge!("spectoncr_http_requests_in_flight", "route" => route).increment(1.0);
    ...
    gauge!("spectoncr_http_requests_in_flight", "route" => route).decrement(1.0);
    counter!("spectoncr_http_requests_total",
        "route" => route, "method" => method_owned.clone(), "status_class" => class)
    .increment(1);
    histogram!("spectoncr_http_request_duration_seconds",
...
async fn get_manifest(
    ...
    record_usage(
        ...
        &params.project,
        &params.name,
        specton_cost::UsageOp::ManifestGet,
        data.len() as i64,
        specton_cost::UsageSrc::Origin,
        200,
...
async fn put_manifest(
    ...
    if !params.reference.starts_with("sha256:") {
        ...
        // when X-SpectonCR-TTL is set. Mirrors the design's project-
        ...
        if let (Some(raw), Some(pool)) = (ttl_header, state.gc_pool.as_ref()) {
            ...
            match specton_ephemeral::parse_ttl_header(now, raw) {
                Ok(spec) => {
                    ...
                    if let Err(e) = r {
                        ...
                        response_headers_extra
                            .push(("X-SpectonCR-TTL-Expires-At".into(), expires_at.to_rfc3339()));
                    }
                }
                Err(e) => {
                    warn!(error = %e, raw = %raw, "ignoring invalid X-SpectonCR-TTL");
                }
            }
        }
    }
    ...
    let parsed_config_digest = specton_gc::extract_config_digest(&body);
    match specton_gc::extract_blob_digests(&body) {
        Ok(blobs) => {
            if let Err(e) = state
                .gc_refcounter
                .add_refs(
    ...
    if let (Some(reg), Some(pool)) = (state.artifact_registry.clone(), state.gc_pool.clone()) {
        ...
        tokio::spawn(async move {
            match reg.validate(&media_type, &body_clone).await {
                Ok(Some(meta)) => {
                    let store = specton_artifact_types::PgArtifactStore::new(pool);
                    use specton_artifact_types::ArtifactStore as _;
    ...
    if let (Some(config_digest), Some(pool)) = (parsed_config_digest, state.gc_pool.clone()) {
        ...
        tokio::spawn(async move {
            ...
            let Some(hint) = specton_rebuild::detect_lineage(&cfg_bytes) else {
                return;
            };
            let r = sqlx::query(
    ...
    record_usage(
        ...
        &params.project,
        &params.name,
        specton_cost::UsageOp::ManifestPut,
        body.len() as i64,
        specton_cost::UsageSrc::Origin,
        201,
        Some(claims.sub.clone()),
...
async fn complete_blob_upload(
    ...
    let lazy_enabled = std::env::var("SPECTONCR_LAZY__ENABLED")
        .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
        .unwrap_or(false);
    if let Some(pool) = state.gc_pool.clone().filter(|_| lazy_enabled) {
        let format = std::env::var("SPECTONCR_LAZY__FORMAT").unwrap_or_else(|_| "estargz".into());
        let layer_digest = expected_digest.clone();
        let tenant = params.tenant.clone();
        let project = params.project.clone();
        let repo = params.name.clone();
        tokio::spawn(async move {
            use specton_lazy::LazyJobStore as _;
            let store = specton_lazy::PgLazyJobStore::new(pool);
            if let Err(e) = store
                .enqueue(
                    &layer_digest,
                    &format,
...
async fn list_tags(
    ...
    let tag_list = specton_common::models::TagList {
        name: repo_name,
        tags,
    };
...
async fn catalog(
    ...
    let catalog_resp = specton_common::models::Catalog { repositories };
...
fn record_usage(
    ...
    project: &str,
    repository: &str,
    op: specton_cost::UsageOp,
    bytes: i64,
    src: specton_cost::UsageSrc,
    status: i32,
    sub: Option<String>,
) {
    let recorder = state.usage_recorder.clone();
    let event = specton_cost::UsageEvent {
        at: chrono::Utc::now(),
...
async fn gc_reconcile(
    ...
    let cfg = specton_gc::ReconcileConfig {
        apply_fix: body.apply,
        max_blobs: body.max,
    };
    let stats = specton_gc::Reconciler::new(pool)
        .reconcile(cfg)
        .await
...
async fn usage_tenant(
    ...
    let granularity = q
        .granularity
        .as_deref()
        .and_then(specton_cost::Granularity::parse)
        .unwrap_or(specton_cost::Granularity::Hour);

    let buckets = specton_cost::UsageReader::new(pool)
...
async fn usage_top_pulled(
    ...
    let rows = specton_cost::UsageReader::new(pool)
...
async fn list_referrers(
    ...
    use specton_lazy::ReferrerStore;
    let store = specton_lazy::PgReferrerStore::new(pool);
...
async fn upload_attestation(
    ...
    // 1. Parse DSSE.
    let (env, stmt) =
        specton_attest::decode_envelope(&body).map_err(|e| RegistryError::ManifestInvalid {
    ...
    let subject_digest = q
        .subject
        .clone()
        .or_else(|| specton_attest::dsse::first_subject_digest(&stmt));
    ...
    let trusted_csv = std::env::var("SPECTONCR_ATTEST__TRUSTED_BUILDERS").unwrap_or_default();
    ...
    let (level, builder_id) =
        specton_attest::slsa::infer_slsa_level(&stmt.predicate_type, &stmt.predicate, &trusted);
    ...
    let envelope_digest = specton_common::storage::sha256_digest(&body);
    let env_path = specton_common::storage::blob_path(
        &params.tenant,
        &params.project,
        &params.name,
        &envelope_digest,
    ...
    //     in SPECTONCR_ATTEST__ED25519_KEYS. Empty / unconfigured ⇒
    //     verified=false (slice-2 advisory mode).
    let verified = {
        use base64::Engine as _;
        let raw = std::env::var("SPECTONCR_ATTEST__ED25519_KEYS").unwrap_or_default();
        let mut verifiers: Vec<Box<dyn specton_attest::Verifier>> = Vec::new();
        for spec in raw.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()) {
            if let Some((keyid, b64)) = spec.split_once(':') {
                match base64::engine::general_purpose::STANDARD.decode(b64.as_bytes()) {
                    Ok(bytes) => match specton_attest::Ed25519Verifier::from_bytes(keyid, &bytes) {
        ...
        if verifiers.is_empty() {
            ...
            matches!(
                specton_attest::verify_envelope(&env, &verifiers),
                Ok(specton_attest::VerifyVerdict::Verified)
            )
        }
    };

    // 5. Persist the row + register a referrer.
    let attestation = specton_attest::store::Attestation {
        id: Uuid::new_v4(),
    ...
    use specton_attest::AttestationStore as _;
    let store = specton_attest::PgAttestationStore::new(pool.clone());
    ...
    use specton_lazy::ReferrerStore as _;
    let r = specton_lazy::Referrer {
        subject_digest: subject_digest.clone(),
    ...
    if let Err(e) = specton_lazy::PgReferrerStore::new(pool).register(&r).await {
...
/// Load scanner config from `SPECTONCR_SCANNER__*` env vars and build the
/// runtime. Returns `Ok(None)` when `SPECTONCR_SCANNER__ENABLED` is not "true".
async fn build_scanner_runtime(
    store: Arc<dyn ObjectStore>,
) -> anyhow::Result<Option<ScannerRuntime>> {
    use std::env;
    let enabled = env::var("SPECTONCR_SCANNER__ENABLED")
        .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
        .unwrap_or(false);
    if !enabled {
        return Ok(None);
    }
    let postgres_url = env::var("SPECTONCR_SCANNER__POSTGRES_URL").map_err(|_| {
        anyhow::anyhow!("SPECTONCR_SCANNER__POSTGRES_URL required when scanner enabled")
    })?;
    let redis_url = env::var("SPECTONCR_SCANNER__REDIS_URL").map_err(|_| {
        anyhow::anyhow!("SPECTONCR_SCANNER__REDIS_URL required when scanner enabled")
    })?;
    let vulndb = env::var("SPECTONCR_SCANNER__VULNDB").unwrap_or_else(|_| "osv".into());
    let vulndb = match vulndb.as_str() {
        "specton" => specton_scanner::config::VulnDbBackend::Specton,
        _ => specton_scanner::config::VulnDbBackend::Osv,
    };
    let ai_enabled = env::var("SPECTONCR_SCANNER__AI_ENABLED")
        .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
        .unwrap_or(true);
    let ai_endpoint = env::var("SPECTONCR_SCANNER__AI_ENDPOINT")
        .unwrap_or_else(|_| "http://127.0.0.1:11434".into());
    let ai_model =
        env::var("SPECTONCR_SCANNER__AI_MODEL").unwrap_or_else(|_| "qwen2.5-coder:7b".into());
    ...
    let cfg = ScannerConfig {
        ...
        ai_endpoint,
        ai_model,
        workers: env::var("SPECTONCR_SCANNER__WORKERS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(2),
        queue_capacity: env::var("SPECTONCR_SCANNER__QUEUE_CAPACITY")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(256),
        result_ttl_secs: env::var("SPECTONCR_SCANNER__RESULT_TTL_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(3600),
        pg_max_connections: env::var("SPECTONCR_SCANNER__PG_MAX_CONNECTIONS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(8),
        ingest_enabled: env::var("SPECTONCR_SCANNER__INGEST_ENABLED")
            .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
            .unwrap_or(true),
        ingest_interval_secs: env::var("SPECTONCR_SCANNER__INGEST_INTERVAL_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(21_600),
        export_prefix: env::var("SPECTONCR_SCANNER__EXPORT_PREFIX")
            .unwrap_or_else(|_| "scanner-exports".into()),
        nvd_enabled: env::var("SPECTONCR_SCANNER__NVD_ENABLED")
            .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
            .unwrap_or(false),
        nvd_api_key: env::var("SPECTONCR_SCANNER__NVD_API_KEY").ok(),
        nvd_bootstrap_window_days: env::var("SPECTONCR_SCANNER__NVD_BOOTSTRAP_WINDOW_DAYS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(30),
        nvd_sleep_between_pages_secs: env::var("SPECTONCR_SCANNER__NVD_SLEEP_BETWEEN_PAGES_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(6),
        ghsa_enabled: env::var("SPECTONCR_SCANNER__GHSA_ENABLED")
            .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
            .unwrap_or(false),
        ghsa_token: env::var("SPECTONCR_SCANNER__GHSA_TOKEN").ok(),
        rate_limit_rpm: env::var("SPECTONCR_SCANNER__RATE_LIMIT_RPM")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(600),
        alerts_webhook_url: env::var("SPECTONCR_SCANNER__ALERTS_WEBHOOK_URL").ok(),
        alerts_format: env::var("SPECTONCR_SCANNER__ALERTS_FORMAT")
            .unwrap_or_else(|_| "generic".into()),
        queue_backend: match env::var("SPECTONCR_SCANNER__QUEUE_BACKEND")
            .unwrap_or_else(|_| "tokio".into())
            .to_lowercase()
            .as_str()
        {
            "postgres" => specton_scanner::config::QueueBackend::Postgres,
            _ => specton_scanner::config::QueueBackend::Tokio,
        },
        enqueue_only: env::var("SPECTONCR_SCANNER__ENQUEUE_ONLY")
            .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
            .unwrap_or(false),
        scan_dedup_enabled: env::var("SPECTONCR_SCANNER__SCAN_DEDUP_ENABLED")
            .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
            .unwrap_or(true),
    };
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("SpectonCR Registry starting up");
    ...
    .record(0.0);
    gauge!("spectoncr_http_requests_in_flight", "route" => "v2_check").set(0.0);

    // Rate limiter — populated by rate_limit_middleware.
    counter!("spectoncr_rate_limit_rejected_total", "tenant" => "anonymous").increment(0);
    ...
    for op in [
        ...
        .increment(0);
        counter!("spectoncr_storage_operation_errors_total", "operation" => op).increment(0);
        histogram!("spectoncr_storage_operation_duration_seconds", "operation" => op).record(0.0);
    ...
    gauge!("spectoncr_circuit_breaker_state", "breaker" => "storage").set(0.0);
    counter!("spectoncr_circuit_breaker_transitions_total",
        "breaker" => "storage", "to" => "open")
    .increment(0);
    counter!("spectoncr_circuit_breaker_rejections_total", "breaker" => "storage").increment(0);

    // Mirror.
    for kind in ["manifest", "blob"] {
        counter!("spectoncr_mirror_cache_misses_total", "kind" => kind).increment(0);
        for outcome in [
            "fetched",
            "not_found",
            "error",
            ...
            counter!("spectoncr_mirror_fetch_total", "kind" => kind, "outcome" => outcome)
                .increment(0);
        }
    }

    // Replication.
    counter!("spectoncr_replication_enqueued_total", "kind" => "manifest").increment(0);
    counter!("spectoncr_replication_enqueued_total", "kind" => "blob").increment(0);
    counter!("spectoncr_replication_enqueued_total", "kind" => "delete").increment(0);
    counter!("spectoncr_replication_enqueue_failures_total").increment(0);
    gauge!("spectoncr_replication_queue_depth").set(0.0);

    // Webhook.
    counter!("spectoncr_webhook_enqueued_total", "event" => "manifest.push").increment(0);
    counter!("spectoncr_webhook_enqueued_total", "event" => "manifest.delete").increment(0);
    counter!("spectoncr_webhook_enqueued_total", "event" => "blob.push").increment(0);
    counter!("spectoncr_webhook_enqueue_failures_total", "event" => "manifest.push").increment(0);

    // Process start-time gauge — Prometheus convention; uptime is computed
    // by Grafana as `time() - spectoncr_process_start_time_seconds`.
    gauge!("spectoncr_process_start_time_seconds").set(chrono::Utc::now().timestamp() as f64);
    ...
    // ── Scanner runtime (optional; enabled by SPECTONCR_SCANNER__ENABLED) ──
    ...
    // Enabled by `SPECTONCR_GC__ONLINE=true`. When enabled, we reuse
    ...
    let gc_online_enabled = std::env::var("SPECTONCR_GC__ONLINE")
        .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
        .unwrap_or(false);
    let (gc_refcounter, gc_reaper_control, gc_pool): (
        Arc<dyn specton_gc::BlobRefCounter>,
        Option<Arc<specton_gc::ReaperControl>>,
        Option<sqlx::PgPool>,
    ) = if gc_online_enabled {
        let pool_opt = if let Some(rt) = scanner_runtime.as_ref() {
            Some(rt.pg.clone())
        } else if let Ok(url) = std::env::var("SPECTONCR_GC__POSTGRES_URL") {
            match specton_db::connect(&url, 4).await {
                Ok(p) => match specton_db::migrate(&p).await {
                    Ok(()) => Some(p),
                    Err(e) => {
            ...
            warn!(
                "SPECTONCR_GC__ONLINE=true but neither scanner nor SPECTONCR_GC__POSTGRES_URL provided; using no-op refcounter"
            );
            None
        };
        match pool_opt {
            Some(pool) => {
                ...
                let reaper_enabled = std::env::var("SPECTONCR_GC__REAPER_ENABLED")
                    .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
                    .unwrap_or(true);

                let control = if reaper_enabled {
                    let control = specton_gc::ReaperControl::new();
                    let cfg = specton_gc::ReaperConfig {
                        grace: std::time::Duration::from_secs(
                            std::env::var("SPECTONCR_GC__REAPER_GRACE_SECS")
                                .ok()
                                .and_then(|v| v.parse::<u64>().ok())
                                .unwrap_or(24 * 3600),
                        ),
                        batch_size: std::env::var("SPECTONCR_GC__REAPER_BATCH")
                            .ok()
                            .and_then(|v| v.parse::<i64>().ok())
                            .unwrap_or(200),
                        idle_sleep: std::time::Duration::from_secs(
                            std::env::var("SPECTONCR_GC__REAPER_IDLE_SLEEP_SECS")
                                .ok()
                                .and_then(|v| v.parse::<u64>().ok())
                                .unwrap_or(30),
                        ),
                        sweep_qps: std::env::var("SPECTONCR_GC__REAPER_QPS")
                            .ok()
                            .and_then(|v| v.parse::<u32>().ok())
                            .unwrap_or(100),
                    };
                    let reaper = specton_gc::ContinuousReaper::new(
                        pool.clone(),
                        store.clone(),
                        cfg,
                        control.clone(),
                ...
                let rc: Arc<dyn specton_gc::BlobRefCounter> =
                    Arc::new(specton_gc::PgBlobRefCounter::new(pool.clone()));
                (rc, control, Some(pool))
            }
            None => (Arc::new(specton_gc::NoopBlobRefCounter), None, None),
        }
    } else {
        (Arc::new(specton_gc::NoopBlobRefCounter), None, None)
    };

    // ── Lazy-pull indexer worker (010 slice 2) ───────────────────────
    // Enabled by SPECTONCR_LAZY__ENABLED. Spawns a worker that drains
    ...
    let lazy_worker_enabled = std::env::var("SPECTONCR_LAZY__ENABLED")
        .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
        .unwrap_or(false);
    if let Some(pool) = gc_pool.clone().filter(|_| lazy_worker_enabled) {
        ...
        let fetcher: Arc<dyn specton_lazy::LayerFetcher> =
            Arc::new(specton_lazy::ObjectStoreLayerFetcher {
                store: store.clone(),
            });
        let indexers: Vec<Arc<dyn specton_lazy::TocIndexer>> =
            vec![Arc::new(specton_lazy::StubEstargzIndexer)];
        let worker_control = specton_lazy::WorkerControl::new();
        let worker = specton_lazy::Worker::new(
            pool.clone(),
            fetcher,
            indexers,
            specton_lazy::WorkerConfig::default(),
    ...
    // storage handle. Kill-switched by SPECTONCR_TTL__REAPER_ENABLED;
    // off by default so existing deployments stay no-op.
    let ttl_reaper_control: Option<Arc<specton_ephemeral::TtlReaperControl>> = {
        let enabled = std::env::var("SPECTONCR_TTL__REAPER_ENABLED")
            .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
            .unwrap_or(false);
        match (enabled, gc_pool.clone()) {
            (true, Some(pool)) => {
                let control = specton_ephemeral::TtlReaperControl::new();
                let cfg = specton_ephemeral::TtlReaperConfig {
                    batch_size: std::env::var("SPECTONCR_TTL__REAPER_BATCH")
                        .ok()
                        .and_then(|v| v.parse().ok())
                        .unwrap_or(100),
                    idle_sleep: std::time::Duration::from_secs(
                        std::env::var("SPECTONCR_TTL__REAPER_IDLE_SLEEP_SECS")
                            .ok()
                            .and_then(|v| v.parse().ok())
                            .unwrap_or(60),
                    ),
                ...
                let reaper =
                    specton_ephemeral::TtlReaper::new(pool, store.clone(), cfg, control.clone());
                tokio::spawn(async move {
                    let _ = reaper.run().await;
                });
            ...
            (true, None) => {
                warn!(
                    "SPECTONCR_TTL__REAPER_ENABLED=true but no Postgres pool; \
    ...
    // Enabled by `SPECTONCR_USAGE__ENABLED=true`. Reuses the GC pool
    // when present (both want the same Postgres). Defaults to a no-op
    // so existing deployments are unaffected.
    let usage_enabled = std::env::var("SPECTONCR_USAGE__ENABLED")
        .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
        .unwrap_or(false);
    // ── Typed artifact validators (016) ──────────────────────────────
    // Enabled by `SPECTONCR_ARTIFACT_TYPES__ENABLED=true`. Slice 1 ships
    // the Helm validator; further types (WASM/model/Terraform) plug
    // in here in subsequent slices. Default off → no-op manifest path.
    let artifact_types_enabled = std::env::var("SPECTONCR_ARTIFACT_TYPES__ENABLED")
        .map(|v| matches!(v.as_str(), "true" | "1" | "yes"))
        .unwrap_or(false);
    let artifact_registry: Option<Arc<specton_artifact_types::ArtifactRegistry>> =
        if artifact_types_enabled {
            info!("typed artifact validators enabled (helm/wasm/model/tfmodule)");
            Some(Arc::new(
                specton_artifact_types::ArtifactRegistry::new()
                    .register(specton_artifact_types::HelmType)
                    .register(specton_artifact_types::WasmType)
                    .register(specton_artifact_types::ModelType)
                    .register(specton_artifact_types::TerraformModuleType),
            ))
        } else {
            None
        };

    let usage_recorder: Arc<dyn specton_cost::UsageRecorder> = if usage_enabled {
        match gc_pool.clone() {
            Some(pool) => {
                info!("usage recorder enabled (postgres-backed)");

                // Spawn drainer (staging → durable). Idempotent + crash-safe.
                let drainer_control = specton_cost::DrainerControl::new();
                let drainer_cfg = specton_cost::DrainerConfig {
                    interval: std::time::Duration::from_secs(
                        std::env::var("SPECTONCR_USAGE__DRAINER_INTERVAL_SECS")
                            .ok()
                            .and_then(|v| v.parse().ok())
                            .unwrap_or(60),
                    ),
                    batch_size: std::env::var("SPECTONCR_USAGE__DRAINER_BATCH")
                        .ok()
                        .and_then(|v| v.parse().ok())
                        .unwrap_or(5000),
                };
                let drainer =
                    specton_cost::Drainer::new(pool.clone(), drainer_cfg, drainer_control.clone());
                tokio::spawn(async move {
                ...
                let rollup_control = specton_cost::RollupControl::new();
                let rollup_cfg = specton_cost::RollupConfig {
                    interval: std::time::Duration::from_secs(
                        std::env::var("SPECTONCR_USAGE__ROLLUP_INTERVAL_SECS")
                            .ok()
                            .and_then(|v| v.parse().ok())
                            .unwrap_or(300),
                    ),
                ...
                let rollup =
                    specton_cost::Rollup::new(pool.clone(), rollup_cfg, rollup_control.clone());
                tokio::spawn(async move {
                ...
                Arc::new(specton_cost::PgUsageRecorder::new(pool))
            }
            None => {
                warn!(
                    "SPECTONCR_USAGE__ENABLED=true but no Postgres pool available; \
                     enable scanner or GC to provide one. Falling back to no-op."
                );
                Arc::new(specton_cost::NoopUsageRecorder)
            }
        }
    } else {
        Arc::new(specton_cost::NoopUsageRecorder)
    ...
    info!(addr = %listen_addr, "SpectonCR Registry listening");
...
mod tests {
    ...
    fn mirror_scope_from_config_allowlist() {
        let cfg = specton_common::config::MirrorScopeConfig {
